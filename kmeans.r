...
library(readxl)
my_data <- read_excel("C:\\Users\\DELL\\Downloads\\EastWestAirlines (1).xlsx",sheet = 2)
...
str(fit)
fit$cluster
final <- data.frame(fit$cluster, my_data) # Append cluster membership

aggregate(my_data[, 1:10], by = list(fit$cluster), FUN = mean)
...
aggregate(my_data[, 2:5], by = list(fit$cluster), FUN = mean)
install.packages("readr")
library(readr)
write_csv(final,"final.csv")
getwd()
...
install.packages('readr')
library(readr)
my_data <- read_csv("C:\\Users\\DELL\\Downloads\\Insurance Dataset.csv")
new_data <- my_data[ ,c(1:5)]
summary(new_data)
sum(is.na(my_data))
sum(is.null(my_data))
...
#boxplot
#boxplot for some features of df norm_data
boxplot(new_data$"Premiums Paid")
boxplot(new_data$Age)
boxplot(new_data$"Days to Renew")
boxplot(new_data$"Claims made")
boxplot(new_data$Income)

#detection of outliers and use winzorisation technique
cunt_Premiums_Paid<- quantile(new_data$"Premiums Paid" , probs = c(.25 , .75))
winso_Premiums_Paid <- quantile(new_data$"Premiums Paid" , probs = c(.01 ,.95) , na.rm = TRUE)
H_Premiums_Paid<- 1.5*IQR(new_data$"Premiums Paid" ,na.rm = TRUE)
new_data$"Premiums Paid"[new_data$"Premiums Paid"<(cunt_Premiums_Paid[1]-H_Premiums_Paid)] <- winso_Premiums_Paid[1]
new_data$"Premiums Paid"[new_data$"Premiums Paid">(cunt_Premiums_Paid[2]+H_Premiums_Paid)] <- winso_Premiums_Paid[2]
boxplot(new_data$"Premiums Paid")

#detection of outliers and use winzorisation technique
cunt_Claims_made<- quantile(new_data$"Claims made" , probs = c(.25 , .75))
winso_Claims_made <- quantile(new_data$"Claims made" , probs = c(.01 ,.90) , na.rm = TRUE)
H_Claims_made<- 1.5*IQR(new_data$"Claims made" ,na.rm = TRUE)
new_data$"Claims made"[new_data$"Claims made"<(cunt_Claims_made[1]-H_Claims_made)] <- winso_Claims_made[1]
new_data$"Claims made"[new_data$"Claims made">(cunt_Claims_made[2]+H_Claims_made)] <- winso_Claims_made[2]
boxplot(new_data$"Claims made")
...
install.packages("ggplot2")
library(ggplot2)
qplot(new_data$"Claims made",new_data$"Premiums Paid",data = new_data,geom = "point")
qplot(new_data$"Days to Renew",Income,data = new_data,color = new_data$"Days to Renew",geom = "point")

#normalise the data
norm_data <- scale(new_data[ ,1:5])
summary(norm_data)
...
aggregate(my_data[, 1:5], by = list(fit$cluster), FUN = mean)
install.packages("readr")
library(readr)
write_csv(final,"final.csv")
getwd()
...
install.packages("readxl")
library(readxl)
my_data <- read_excel("C:\\Users\\DELL\\Downloads\\Telco_customer_churn (1).xlsx")
...
twss <- NULL
for (i in 2:8) {
  twss <- c(twss, kmeans(new_data_dummy, centers = i)$tot.withinss)
}
twss
...
# 3 Cluster Solution
fit <- kmeans(new_data_dummy, 3) 
str(fit)
fit$cluster
final <- data.frame(fit$cluster, my_data) # Append cluster membership

aggregate(my_data[, 1:24], by = list(fit$cluster), FUN = mean)
install.packages("readr")
library(readr)
write_csv(final,"final.csv")
getwd()
